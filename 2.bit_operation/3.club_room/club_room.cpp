...
int calculate_all_possible_cases() {
    ...
    while (true) {
        unordered_map<int, int>& current_table = (table_flag != 0 ? table_b : table_a);
        unordered_map<int, int>& next_table = (table_flag != 0 ? table_a : table_b);
        ...
        for (auto it = current_table.begin(); it != current_table.end();) {
            ...
            for (int key : subset_flag_set) {
                next_table[key] = next_table.find(key) != next_table.end()
                                    ? (next_table[key] + it->second) %  1000000007 : next_table[key] = it->second;
            }
